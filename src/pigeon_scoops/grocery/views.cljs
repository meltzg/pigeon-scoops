...
(defui grocery-unit-row [{:keys [unit]}]
  (let [{:keys [constants]} (use-constants)
    ...
    ($ TableRow
       ...
       ($ TableCell
          ($ InputNumber {:value          (:grocery-unit/unit-cost unit)
                          :on-change     #(set-unit! (assoc unit :grocery-unit/unit-cost %))}))
       (for [[idx [value-key type-key option-key]]
         ...
         ($ TableCell {:key idx}
            ($ Stack {:direction "row" :spacing 1}
               ($ InputNumber {:value          (value-key unit)
                               :on-change     #(set-unit! (assoc unit value-key %))})
               ($ FormControl
