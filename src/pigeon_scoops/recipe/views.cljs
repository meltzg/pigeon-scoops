    ...
    ($ TableRow
       ...
       ($ TableCell
          ($ Stack {:direction "row" :spacing 1}
             ($ InputNumber {:value          (:ingredient/amount ingredient)
                             :on-change     #(set-ingredient! (assoc ingredient :ingredient/amount %))})
             ($ FormControl
    ...
    ($ Stack {:direction "column" :spacing 1 :sx (clj->js {:width "100%"})}
       ...
       (when (uuid? (:recipe/id editable-recipe))
         ($ Stack {:direction "row" :spacing 0.5}
            ($ Typography {:sx (clj->js {:display        "flex"
                                         ...
                                         :height         "100%"})}
               "Scale Amount")
            ($ InputNumber {:value          scaled-amount
       ...
       ($ Stack {:direction "row" :spacing 0.5}
          ($ Typography {:sx (clj->js {:display        "flex"
                                       ...
                                       :height         "100%"})}
             "Amount")
          ($ InputNumber {:value          (:recipe/amount editable-recipe)
                          :on-change     #(do (prn "asdf" %) (set-editable-recipe! (assoc editable-recipe :recipe/amount %)))})
